...
| Static payload torque      |                          ~98 N·m |
| Target output torque       |                         ~196 N·m |
...
where $Z_{11}$ is the number of teeth of the first stage cycloid disc (11) and $Z_{12}$ is the number of rollers of the stationary ring gear of the first stage (12).

The second stage transmission ratio is calculated based on the following expression:

$$
u_2 = -\frac{Z_{12}}{Z_{u2} - Z_{u1}}
$$

where $Z_{21}$ is the number of teeth of the second stage cycloid disc (10) and $Z_{22}$ is the number of rollers of the rotatable ring gear of the second stage (11).

In the case of ``one tooth difference'' cycloidal drive

$$
Z_{12} - Z_{11} = 1, \qquad Z_{u2} - Z_{u1} = 1,
$$

as is the case here, the final expression for the speed reducer ratio $u_{\mathrm{CR}}$ is

$$
u_{\mathrm{CR}} = Z_{11} * Z_{u2}
= 11 * 12
= 121
$$
...
A segmented shaft prtined out of PETG is used to transmit torque between the motor and gearbox. The segmented pieces snap-fit to each other and are further reinforced via screws.
...
Inspired by https://www.researchgate.net/publication/235992854_A_New_Design_of_a_Two-Stage_Cycloidal_Speed_Reducer, I decided to develop this nontraditional cycloidal drive for a robotic arm.
...
* Oriented pieces in specific printing orientation to maximize part durability
* Redesigned and modified CAD to compensate for printing tolerances or printing orientation
* Re-printed pieces in varying material (PETG to Nylon, for example) to better suit the piece's function
